-- Invoice/VAT info captured per order at intake time.
-- Optional: customer can opt-in to receive a VAT invoice, supplying company name + tax code + address + email.
...
-- ── record_dealer_order: append invoice params (all optional, NULL when not requested) ──
DROP FUNCTION IF EXISTS public.record_dealer_order(UUID, INT, TEXT, TEXT, TEXT, DATE);
...
BEGIN
    ...
    IF p_invoice_required THEN
        ...
        END IF;
        IF length(coalesce(trim(p_invoice_address), '')) < 5 THEN
            RAISE EXCEPTION 'Địa chỉ xuất hoá đơn bắt buộc';
        END IF;
    END IF;
    ...
    INSERT INTO public.orders (
        ...
        invoice_required, invoice_company_name, invoice_tax_code,
        ...
        CASE WHEN p_invoice_required THEN trim(p_invoice_address) END,
...
REVOKE ALL ON FUNCTION public.record_dealer_order(
    UUID, INT, TEXT, TEXT, TEXT, DATE, BOOLEAN, TEXT, TEXT, TEXT, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.record_dealer_order(
    UUID, INT, TEXT, TEXT, TEXT, DATE, BOOLEAN, TEXT, TEXT, TEXT, TEXT) TO authenticated;

-- ── submit_public_order: append invoice params (same shape, optional) ──
DROP FUNCTION IF EXISTS public.submit_public_order(TEXT, UUID, INT, TEXT, TEXT, TEXT);
...
REVOKE ALL ON FUNCTION public.submit_public_order(
    TEXT, UUID, INT, TEXT, TEXT, TEXT, BOOLEAN, TEXT, TEXT, TEXT, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.submit_public_order(
    TEXT, UUID, INT, TEXT, TEXT, TEXT, BOOLEAN, TEXT, TEXT, TEXT, TEXT) TO anon, authenticated;
