...
-- Supervisor branch relies on profiles' supervisor_only_for_dealer CHECK
-- (supervisor_id is only ever set on dealers), so CRM visibility for the
-- supervisor case depends on that constraint holding.
...
GRANT USAGE ON SEQUENCE public.crm_account_code_seq TO authenticated;
...
-- Always assigns the code on INSERT: no caller ever supplies an explicit
-- code, so honoring a client-supplied value would just be dead weight that
-- risks colliding with a future sequence value.
...
-- Contact writes must also check the parent account's ownership (not
-- crm_owner_visible): FK checks bypass RLS, so without this a dealer who
-- learns another dealer's crm_accounts.id (e.g. from a URL) could attach or
-- edit contacts on someone else's customer. A supervisor may read a branch
-- account but must not write contacts on it, hence the ownership test here
-- rather than the visibility helper.
