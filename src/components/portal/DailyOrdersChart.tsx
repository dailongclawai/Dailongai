    ...
    <div className="relative overflow-hidden rounded-2xl border border-[#49443f]/40 bg-gradient-to-br from-[#1e2022] via-[#1e2022] to-[#0c0e10] p-6">
      ...
      <div className="relative flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex gap-6 text-right">
          <div>
            <p className="text-[10px] uppercase tracking-wider text-[#e2e2e6]/40">{t('portal.components.dailyOrdersChart.avg_per_day')}</p>
            <p className="mt-0.5 font-mono text-sm font-semibold tabular-nums">{fmtVnd(stats.avgDaily)} ₫</p>
          </div>
          <div>
            <p className="text-[10px] uppercase tracking-wider text-[#e2e2e6]/40">{t('portal.components.dailyOrdersChart.active_days')}</p>
            <p className="mt-0.5 font-mono text-sm font-semibold tabular-nums">
              {stats.activeDays}<span className="text-[#e2e2e6]/40">/{days}</span>
            </p>
          </div>
          <div>
      ...
      <div className="relative mt-6">
        ...
        <div className="relative h-44">
          ...
          {/* Y-axis label */}
          <div className="pointer-events-none absolute -left-1 top-0 font-mono text-[9px] uppercase tracking-wider text-[#e2e2e6]/30">
            {fmtVnd(stats.maxRev)}₫
          </div>
        ...
        <div className="mt-2 flex gap-[2px]">
          {buckets.map((b, idx) => {
            ...
            return (
              <div key={b.key} className="flex-1 text-center font-mono text-[9px] tabular-nums text-[#e2e2e6]/30">
