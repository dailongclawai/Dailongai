...
export function OrderForm({ userId: _userId }: { userId: string }) {
  ...
  if (done) {
    ...
    return (
      <div className="mx-auto max-w-2xl space-y-5">
        <div className="rounded-2xl border border-orange-500/30 bg-[#1a1c1f] p-5 text-center">
          <span className="material-symbols-outlined text-orange-400 text-[40px]">check_circle</span>
          <h2 className="mt-2 font-headline text-2xl">{t('portal.components.orderForm.done_title_prefix')} {done.customer}</h2>
          <p className="mt-1 text-xs text-[#b3aca8]">{t('portal.components.orderForm.done_subtitle')}</p>
        </div>

        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div className="rounded-xl border border-[#49443f] bg-[#1a1c1f] px-4 py-3">
            <p className="text-[10px] uppercase tracking-wider text-[#b3aca8]">{t('portal.components.orderForm.order_code')}</p>
            <p className="mt-0.5 font-mono text-lg font-bold tabular-nums text-[#ff8a50]">{memo}</p>
          </div>
          <div className="rounded-xl border border-[#49443f] bg-[#1a1c1f] px-4 py-3">
            <p className="text-[10px] uppercase tracking-wider text-[#b3aca8]">{t('portal.components.orderForm.created_at')}</p>
        ...
        <div className="flex gap-3">
          <button
            ...
            className="flex-1 rounded-lg border border-[#49443f] bg-[#1a1c1f] py-3 text-sm font-bold text-[#e2e2e6] hover:bg-[#1e2023]"
          >
            {t('portal.components.orderForm.new_order_button')}
          </button>
          <Link
            href="/portal/dealer/commission"
            className="flex flex-1 items-center justify-center rounded-lg bg-[#e8692a] py-3 text-sm font-bold text-white hover:bg-[#e8692a]/90"
          >
  ...
  return (
    <form
      ...
      className="mx-auto max-w-2xl space-y-5 rounded-2xl border border-[#49443f]/40 bg-[#1a1c1e] p-6"
    >
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
        <div className="sm:col-span-2">
          <label className="block text-[11px] uppercase tracking-wider text-[#b3aca8] mb-1.5">{t('portal.components.orderForm.label_product')}</label>
          <ProductPicker models={models} selectedId={modelId} onSelect={setModelId} />
        </div>

        <div>
          <label className="block text-[11px] uppercase tracking-wider text-[#b3aca8] mb-1.5">{t('portal.components.orderForm.label_quantity')}</label>
          <div className="flex items-stretch overflow-hidden rounded-lg border border-[#49443f]/40 bg-[#0c0e11]">
            <button
              type="button"
              onClick={() => setQuantity((q) => Math.max(1, q - 1))}
              className="flex w-12 items-center justify-center text-lg font-bold text-[#b3aca8] hover:bg-[#1e2023] hover:text-[#ff8a50]"
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
        <div>
          <label className="block text-[11px] uppercase tracking-wider text-[#b3aca8] mb-1.5">{t('portal.components.orderForm.label_customer')}</label>
          <input
            type="text"
            value={customer}
            ...
            className="w-full rounded-lg border border-[#49443f]/40 bg-[#0c0e11] px-3 py-2.5 text-sm outline-none focus:border-[#ff8a50]"
          />
        </div>
        <div>
          <label className="block text-[11px] uppercase tracking-wider text-[#b3aca8] mb-1.5">{t('portal.components.orderForm.label_phone')}</label>
          <input
            type="tel"
            value={phone}
            ...
            className="w-full rounded-lg border border-[#49443f]/40 bg-[#0c0e11] px-3 py-2.5 text-sm font-mono outline-none focus:border-[#ff8a50]"
          />
        </div>
      </div>

      <div className="rounded-lg border border-[#49443f]/30 bg-[#0f1113] p-4">
        <p className="mb-3 text-[11px] uppercase tracking-[0.2em] text-[#ff8a50] font-bold">{t('portal.components.orderForm.shipping_address')}</p>
        <AddressPicker value={address} onChange={setAddress} />
      </div>

      <InvoiceFieldsSection value={invoice} onChange={setInvoice} />

      <div className="flex items-center gap-2 rounded-lg border border-[#49443f]/40 bg-[#0c0e11] px-3 py-2.5 text-xs text-[#b3aca8]">
        <span className="material-symbols-outlined text-[16px] text-[#ff5625]">schedule</span>
        {t('portal.components.orderForm.auto_record_note')}
      </div>

      <div className="rounded-lg border border-[#49443f]/40 bg-[#0c0e11] p-4">
        <div className="flex items-center justify-between text-sm">
          <span className="text-[#b3aca8]">{t('portal.components.orderForm.unit_price')}</span>
          <span className="font-mono tabular-nums text-[#e2e2e6]">{fmtVnd(unitPrice)} ₫</span>
        </div>
        <div className="mt-1 flex items-center justify-between text-sm">
          <span className="text-[#b3aca8]">{t('portal.components.orderForm.label_quantity')}</span>
          <span className="font-mono tabular-nums text-[#e2e2e6]">× {quantity}</span>
        </div>
        <div className="mt-3 flex items-center justify-between border-t border-[#49443f]/30 pt-3">
          <span className="text-[11px] uppercase tracking-wider text-[#b3aca8]">{t('portal.components.orderForm.total_amount')}</span>
      ...
      <button
        type="submit"
        disabled={busy}
        className="w-full rounded-lg bg-[#e8692a] py-3 font-bold text-white shadow-lg  transition-all hover:bg-[#e8692a]/90 active:scale-95 disabled:opacity-50"
      >
