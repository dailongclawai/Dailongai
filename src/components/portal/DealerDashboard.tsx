  ...
  return (
    <div className="space-y-12 py-4">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-[11px] uppercase tracking-[0.3em] text-[#ffb5a1]">Số 05 / 2026 · Báo cáo tháng</p>
        ...
        <Link href="/portal/dealer/orders/new" className="rounded-full bg-[#ff5626] px-6 py-2.5 text-sm font-medium text-white transition-colors glow-primary-hover hover:bg-[#ff5626]/90">
          + Ghi nhận đơn mới
        </Link>
      </div>
      ...
      <section className="grid grid-cols-1 gap-6 md:grid-cols-12 md:gap-10">
        <div className="md:col-span-7">
          <p className="text-xs uppercase tracking-[0.25em] text-[#fadcd5]/50">Doanh số tháng này</p>
          <p className="mt-3 font-headline text-[72px] leading-[0.85] tracking-tighter md:text-[140px]">
            ...
            <span className="ml-2 align-top font-mono text-3xl tabular-nums text-[#ffb5a1]">
              {monthSales >= 1_000_000 ? 'tr ₫' : '₫'}
            </span>
          </p>
          <p className="mt-4 text-sm text-[#fadcd5]/60">
            <span className="font-mono tabular-nums">{ordersDone}</span> máy đã chốt tháng này
          </p>
        </div>

        <div className="space-y-4 md:col-span-5">
          <div className="overflow-hidden rounded-2xl border border-[#ffb5a1]/25 bg-gradient-to-br from-[#ff5626]/15 to-[#2c1c17] p-6">
            <p className="text-[10px] uppercase tracking-[0.3em] text-[#ffb5a1]">Kỳ chi trả kế tiếp</p>
            <p className="mt-3 font-headline text-5xl leading-none">
              05<span className="text-[#ffb5a1]">–</span>10<span className="ml-2 text-xl text-[#fadcd5]/60">/06</span>
            </p>
            <p className="mt-3 text-sm text-[#fadcd5]/80">Tự động chi 5–10 hàng tháng cho mọi đơn approved</p>
            <p className="mt-2 text-[11px] text-[#fadcd5]/50">
              Đợt này: <span className="font-mono tabular-nums">{fmtShortVnd(commissionPending)}</span> VND dự kiến
            </p>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-xl border border-[#5b4039]/40 bg-[#2c1c17] p-4">
              <p className="text-[10px] uppercase tracking-[0.2em] text-[#fadcd5]/50">Đơn chờ duyệt</p>
              <p className="mt-2 font-mono text-3xl font-medium tabular-nums">{ordersPending}</p>
            </div>
            <div className="rounded-xl border border-[#5b4039]/40 bg-[#2c1c17] p-4">
              <p className="text-[10px] uppercase tracking-[0.2em] text-[#fadcd5]/50">Đã chốt</p>
      ...
      <section className="rounded-3xl border border-[#5b4039]/40 bg-[#2c1c17] p-8">
        <div className="flex items-end justify-between">
          <div>
            <p className="text-[11px] uppercase tracking-[0.3em] text-[#ffb5a1]">Bậc hoa hồng năm 2026</p>
            <h2 className="mt-1 font-headline text-2xl">
              {currentTier.label} · {currentTier.percent}%
            </h2>
          </div>
          <p className="text-xs text-[#fadcd5]/60">
            YTD: <span className="font-mono tabular-nums">{unitsYtd}</span> máy
          </p>
        </div>
        <div className="mt-10">
          <div className="relative h-1 rounded-full bg-[#372621]">
            <div
              className="absolute left-0 top-0 h-1 rounded-full bg-gradient-to-r from-[#ff5626] to-[#ffb5a1]"
              ...
              >
                <div
                  className={`h-4 w-4 rounded-full border-2 ${
                    unitsYtd >= t.minUnits ? 'border-[#ffb5a1] bg-[#ff5626]' : 'border-[#5b4039]/70 bg-[#1e100c]'
                  }`}
                />
              </div>
            ))}
          ...
          <div className="mt-12 grid grid-cols-3 text-sm">
            {tiers.map((t) => (
              <div key={t.id} className={t.id === currentTier.id ? 'opacity-100' : 'opacity-50'}>
                <p className="text-[10px] uppercase tracking-[0.25em] text-[#fadcd5]/50">{t.label}</p>
                <p className="mt-1 font-mono text-lg font-medium tabular-nums">{t.minUnits}+ máy</p>
                <p className={`font-headline text-2xl ${unitsYtd >= t.minUnits ? 'text-[#ffb5a1]' : 'text-[#fadcd5]'}`}>
                  {t.percent}%
                </p>
              </div>
            ))}
          ...
          {unitsToNext > 0 && (
            <div className="mt-8 flex items-center gap-3 rounded-xl bg-[#ff5626]/10 px-4 py-3 text-sm">
              <span className="font-semibold text-[#ffb5a1]">▲</span>
              <span>
      ...
      <section>
        <div className="flex items-baseline justify-between border-b border-[#5b4039]/40 pb-2">
          <div>
            <p className="text-[10px] uppercase tracking-[0.3em] text-[#ffb5a1]">Hoạt động</p>
            <h2 className="mt-1 font-headline text-3xl">Đơn gần đây</h2>
          </div>
        </div>
        {orders.length === 0 ? (
          <div className="mt-6 rounded-xl border-2 border-dashed border-[#5b4039]/50 p-12 text-center text-sm text-[#fadcd5]/60">
            <p>Chưa có đơn nào ghi nhận.</p>
          </div>
        ) : (
          <div className="mt-6 divide-y divide-[#5b4039]/30">
            {orders.slice(0, 10).map((o, i) => (
              <div key={o.id} className="flex flex-wrap items-center gap-2 py-3 text-sm md:grid md:grid-cols-12 md:gap-4">
                <span className="font-mono tabular-nums text-[#fadcd5]/30 md:col-span-1">{String(i + 1).padStart(2, '0')}</span>
                <span className="font-medium md:col-span-4">{o.customer_name}</span>
                <span className="font-mono tabular-nums text-[#fadcd5]/60 md:col-span-3">{o.serial_number}</span>
                <span className="text-right font-mono tabular-nums md:col-span-2">{fmtShortVnd(o.sale_price)}</span>
                <span className="text-right text-xs uppercase tracking-wider text-[#ffb5a1] md:col-span-2">{o.status}</span>
              </div>
            ))}
          </div>
        )}
            ...
            {payouts.slice(0, 12).map((p) => (
              <div key={p.id} className="flex items-center justify-between gap-4 py-3 text-sm">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-mono tabular-nums text-xs text-[#fadcd5]/60">
