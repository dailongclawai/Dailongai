...
const COLUMNS: { key: ColumnKey; label: string; tone: string; statuses: Order['status'][] }[] = [
  { key: 'pending', label: 'Chờ duyệt', tone: 'text-[#f59e0b] border-[#f59e0b]/40', statuses: ['pending'] },
  { key: 'approved', label: 'Đã duyệt', tone: 'text-[#3b82f6] border-[#3b82f6]/40', statuses: ['approved'] },
  { key: 'paid', label: 'Đã thanh toán', tone: 'text-[#10b981] border-[#10b981]/40', statuses: ['paid'] },
  { key: 'closed', label: 'Từ chối / Huỷ', tone: 'text-[#9ca3af] border-[#9ca3af]/40', statuses: ['rejected', 'voided'] },
];
                ...
                {items.length === 0 ? (
                  ...
                  return (
                    <li key={o.id} className="rounded-xl border border-[#1f2937] bg-[#11151a] p-3 text-sm">
                      ...
                      {dealerName && (
                        <p className="mt-0.5 truncate text-[11px] text-[#9ca3af]">ĐL: {dealerName}</p>
                      )}
