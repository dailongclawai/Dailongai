        ...
        <div className="flex flex-wrap gap-x-5 sm:gap-x-8 gap-y-2 sm:justify-end">
          {navLinks.map((link) => (
            <a key={link.href} href={link.href}
              className="inline-flex items-center py-1.5 text-[9px] sm:text-[10px] uppercase tracking-[0.15em] sm:tracking-[0.2em] font-normal text-secondary hover:text-tertiary transition-colors duration-200">
              {link.label}
            </a>
          ))}
          ...
          <a href={contactInfo.facebook} target="_blank" rel="noopener noreferrer" aria-label="Facebook"
            className="inline-flex items-center p-1.5 -m-1.5 text-secondary hover:text-tertiary transition-colors duration-200">
