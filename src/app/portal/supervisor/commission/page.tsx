...
function overrideLabel(pct: number | null): { name: string; cls: string } {
  ...
  return { name: `Override ${pct}%`, cls: 'bg-[#00daf3]/10 text-[#00daf3] border-[#00daf3]/20' };
}
...
export default function SupervisorCommissionPage() {
  ...
  const cards = [
    ...
    { key: 'pending', icon: 'hourglass_empty', label: 'Đơn team đang chờ', val: stats.pendingVal, cnt: stats.pendingCnt, cls: 'text-[#00daf3] bg-[#00daf3]/10 border-[#00daf3]/20' },
  ...
  return (
    <PortalShell variant="supervisor">
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-[11px] uppercase tracking-[0.3em] text-[#ff5625]">Override hoa hồng từ đội</p>
          <h1 className="mt-1 font-headline text-3xl">Sổ hoa hồng supervisor</h1>
          <p className="mt-1 text-sm text-[#a0a0a8]">Mọi đơn do đại lý trong nhánh chốt — kèm phần override bạn nhận.</p>
        </div>
      ...
      <div className="portal-glass mb-8 grid grid-cols-1 gap-4 rounded-2xl p-4 sm:grid-cols-12 sm:items-end">
        ...
        <div className="sm:col-span-5">
          ...
          <div className="relative">
            ...
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Tìm serial / đại lý / khách hàng…" className="w-full rounded-xl border border-[#3d3f41]/40 bg-[#1a1c1e] py-2.5 pl-11 pr-4 text-sm text-[#e2e2e5] placeholder:text-[#a0a0a8]/40 focus:ring-1 focus:ring-[#ff5625] outline-none" />
          </div>
        </div>
      </div>
        ...
        <div className="portal-scroll overflow-x-auto">
          <table className="w-full min-w-[920px] text-left">
            ...
            <tbody className="divide-y divide-[#3d3f41]/20">
              {filtered.length === 0 ? (
                <tr><td colSpan={8} className="px-6 py-12 text-center text-sm text-[#a0a0a8]/60">Không có đơn phù hợp.</td></tr>
              ) : filtered.map((r) => {
                const st = statusOf(r);
                const pct = rateOf(r);
                      ...
                      <tr className="bg-[#0c0e10]/50">
                        <td colSpan={8} className="px-12 py-6">
                          <div className="grid grid-cols-2 gap-8 sm:grid-cols-4">
                            ...
                            <div className="space-y-1">
                              <p className="text-[11px] uppercase text-[#a0a0a8]">Người phụ trách</p>
      ...
      {myRequests.length > 0 && (
        <div className="mt-10 overflow-hidden rounded-2xl border border-[#3d3f41]/40 bg-[#1e2022]">
          <div className="border-b border-[#3d3f41]/40 px-6 py-4">
            ...
            <p className="text-xs text-[#a0a0a8]/70">Lịch sử các lần yêu cầu rút override.</p>
          </div>
      ...
      {reqOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => !reqBusy && setReqOpen(false)}>
          <div className="w-full max-w-md rounded-2xl border border-[#3d3f41]/40 bg-[#1e2022] p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="mt-1 text-sm text-[#a0a0a8]">Số dư override đã duyệt: <span className="font-mono font-bold text-emerald-400">{fmtVnd(stats.approvedVal)} ₫</span></p>
            <div className="mt-4 space-y-3">
              ...
              <div>
                ...
                <textarea value={reqNotes} onChange={(e) => setReqNotes(e.target.value)} rows={3} className="w-full resize-none rounded-lg border border-[#3d3f41]/50 bg-[#1a1c1e] px-3 py-2 text-sm text-[#e2e2e5] outline-none focus:border-[#ff5625]" />
              </div>
            </div>
