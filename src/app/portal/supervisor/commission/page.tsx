...
function statusOf(r: LedgerRow): { label: string; cls: string; dot: string; bucket: Exclude<Bucket, 'all'> } {
  ...
  if (r.status === 'voided' || r.commission?.voided_at) return { label: 'Đã hủy', cls: 'text-[#e4beb4] bg-[#372621] border-[#5b4039]/40', dot: 'bg-[#e4beb4]', bucket: 'rejected' };
  ...
  if (r.commission) return { label: 'Đã duyệt · chờ chi', cls: 'text-[#84cfff] bg-[#84cfff]/10 border-[#84cfff]/20', dot: 'bg-[#84cfff]', bucket: 'approved' };
  ...
  return { label: r.status, cls: 'text-[#e4beb4] bg-[#372621] border-[#5b4039]/40', dot: 'bg-[#e4beb4]', bucket: 'pending' };
}
    ...
    <PortalShell variant="supervisor">
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={openRequest} className="flex items-center gap-2 rounded-lg bg-[#ff5626] px-4 py-2 text-sm font-bold text-white shadow-lg shadow-[#ff5626]/20 active:scale-95">
      ...
      <div className="portal-glass mb-8 grid grid-cols-1 gap-4 rounded-2xl p-4 sm:grid-cols-12 sm:items-end">
        ...
        <div className="sm:col-span-4">
          <label className="mb-1.5 ml-1 block text-[11px] uppercase tracking-wider text-[#e4beb4]">Khoảng thời gian</label>
          <div className="flex items-center gap-2 rounded-xl border border-[#5b4039]/40 bg-[#271814] px-3 py-2">
            <span className="material-symbols-outlined text-[18px] text-[#e4beb4]">calendar_today</span>
            <input type="date" value={from} onChange={(e) => setFrom(e.target.value)} className="w-full bg-transparent text-sm text-[#fadcd5] outline-none [color-scheme:dark]" />
            <span className="text-[#e4beb4]">–</span>
            <input type="date" value={to} onChange={(e) => setTo(e.target.value)} className="w-full bg-transparent text-sm text-[#fadcd5] outline-none [color-scheme:dark]" />
          </div>
        </div>
                        ...
                        <td colSpan={8} className="px-12 py-6">
                          <div className="grid grid-cols-2 gap-8 sm:grid-cols-4">
                            ...
                            <div className="space-y-1">
                              ...
                              <p className="font-mono text-sm text-[#ffb5a1] break-all">{r.commission?.payment_proof_url || '—'}</p>
                            </div>
                          </div>
                        </td>
                      </tr>
        ...
        <div className="mt-10 overflow-hidden rounded-2xl border border-[#5b4039]/40 bg-[#2c1c17]">
          ...
          <table className="w-full text-left text-sm">
            <thead className="text-[11px] uppercase tracking-wider text-[#e4beb4]">
              <tr className="border-b border-[#5b4039]/30">
            ...
            <tbody className="divide-y divide-[#5b4039]/20">
              {myRequests.map((req) => (
                <tr key={req.id}>
                  <td className="px-6 py-3 font-mono text-xs tabular-nums text-[#e4beb4]">{new Date(req.created_at).toLocaleString('vi-VN')}</td>
                  <td className="px-6 py-3 text-right font-mono font-bold tabular-nums">{fmtVnd(Number(req.amount))} ₫</td>
                  <td className="px-6 py-3 text-[#e4beb4]">{req.notes || '—'}</td>
                  ...
                  <td className="px-6 py-3 text-xs text-[#e4beb4]">{req.processed_at ? new Date(req.processed_at).toLocaleString('vi-VN') : '—'}{req.processor_notes ? ` · ${req.processor_notes}` : ''}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="w-full max-w-md rounded-2xl border border-[#5b4039]/40 bg-[#2c1c17] p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mt-4 space-y-3">
              <div>
                ...
                <input type="number" min={1} max={stats.approvedVal} value={reqAmount} onChange={(e) => setReqAmount(e.target.value)} className="w-full rounded-lg border border-[#5b4039]/50 bg-[#271814] px-3 py-2 font-mono text-sm tabular-nums text-[#fadcd5] outline-none focus:border-[#ffb5a1]" />
              </div>
              <div>
            ...
            <div className="mt-5 flex justify-end gap-3">
              <button onClick={() => setReqOpen(false)} disabled={reqBusy} className="rounded-lg border border-[#5b4039]/50 px-4 py-2 text-sm text-[#fadcd5] hover:bg-[#372621]">Huỷ</button>
              <button onClick={submitRequest} disabled={reqBusy} className="rounded-lg bg-[#ff5626] px-5 py-2 text-sm font-bold text-white hover:bg-[#ff5626]/90 disabled:opacity-50">{reqBusy ? 'Đang gửi…' : 'Gửi yêu cầu'}</button>
            </div>
          </div>
        </div>
      )}
