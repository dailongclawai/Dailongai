...
export default function TeamDealerDetail() {
  ...
  return (
    <PortalShell variant="supervisor">
      <Link href="/portal/supervisor" className="text-xs text-[#e2e2e6]/60 hover:text-[#ff8a50]">{t('portal.supervisor.team.back_to_team')}</Link>
      <h1 className="mt-3 font-headline text-3xl">{t('portal.supervisor.team.heading')}</h1>
      <div className="mt-6 grid grid-cols-4 gap-4">
        <div className="rounded-xl border border-[#49443f]/40 bg-[#1a1c1f] p-4">
          <p className="text-[10px] uppercase tracking-[0.2em] text-[#e2e2e6]/50">{t('portal.supervisor.team.stat.units_month')}</p>
          <p className="mt-2 font-mono text-2xl font-medium tabular-nums">{summary?.units_ytd ?? 0}</p>
        </div>
        <div className="rounded-xl border border-[#49443f]/40 bg-[#1a1c1f] p-4">
          <p className="text-[10px] uppercase tracking-[0.2em] text-[#e2e2e6]/50">{t('portal.supervisor.team.stat.month_sales')}</p>
          <p className="mt-2 font-mono text-2xl font-medium tabular-nums">{fmtVnd(Number(summary?.month_sales ?? 0))}</p>
        </div>
        <div className="rounded-xl border border-[#49443f]/40 bg-[#1a1c1f] p-4">
          <p className="text-[10px] uppercase tracking-[0.2em] text-[#e2e2e6]/50">{t('portal.supervisor.team.stat.orders_pending')}</p>
          <p className="mt-2 font-mono text-2xl font-medium tabular-nums">{summary?.orders_pending ?? 0}</p>
        </div>
        <div className="rounded-xl border border-[#49443f]/40 bg-[#1a1c1f] p-4">
          <p className="text-[10px] uppercase tracking-[0.2em] text-[#e2e2e6]/50">{t('portal.supervisor.team.stat.closed')}</p>
      ...
      <div className="mt-8 grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border border-[#ff5625]/30 bg-[#ff5625]/5 p-6">
          <p className="text-[11px] uppercase tracking-[0.3em] text-[#ff5625]">{t('portal.supervisor.team.tier.auto_label')}</p>
          <div className="mt-3 flex items-end justify-between">
            <div>
              ...
              <p className="mt-1 text-[11px] text-[#e2e2e6]/50">
                {t('portal.supervisor.team.tier.closed_prefix')} <span className="font-mono tabular-nums text-[#e2e2e6]/80">{currentCommission?.units_ytd ?? 0}</span> {t('portal.supervisor.team.tier.closed_suffix')}
              </p>
            </div>
            <div className="text-right">
              <p className="font-mono text-3xl font-bold tabular-nums text-[#ff5625]">{Number(currentCommission?.tier_percent ?? 15)}%</p>
              <p className="text-[10px] text-[#e2e2e6]/40">{t('portal.supervisor.team.tier.of_sale_price')}</p>
            </div>
          </div>
          <p className="mt-3 text-[10px] text-[#e2e2e6]/40">{t('portal.supervisor.team.tier.thresholds')}</p>
        </div>

        {/* Fixed override */}
        <div className="rounded-2xl border border-[#ff8a50]/30 bg-[#ff8a50]/5 p-6">
          <div className="flex items-center justify-between">
            <p className="text-[11px] uppercase tracking-[0.3em] text-[#ff8a50]">{t('portal.supervisor.team.fixed.label')}</p>
            {currentCommission?.source === 'fixed' && (
              <span className="rounded-full bg-[#ff8a50]/15 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-[#ff8a50]">{t('portal.supervisor.team.fixed.active_badge')}</span>
            )}
          </div>
          <div className="mt-3 flex items-baseline justify-between">
            <span className="text-[11px] text-[#e2e2e6]/50">{t('portal.supervisor.team.fixed.amount_per_unit')}</span>
            <span className="font-mono text-2xl font-bold tabular-nums text-[#ff8a50]">
              {amount.toLocaleString('vi-VN')} <span className="text-base text-[#e2e2e6]/50">₫</span>
            </span>
          </div>
          <input
            type="range"
            ...
            className="mt-2 w-full accent-[#ff8a50]"
          />
          <div className="mt-1 flex justify-between font-mono text-[10px] tabular-nums text-[#e2e2e6]/40">
          ...
          <div className="mt-3 flex items-center gap-2">
            <input
              ...
              value={amount}
              onChange={(e) => setAmount(Number(e.target.value))}
              className="flex-1 rounded-lg border border-[#49443f]/50 bg-[#1a1c1e] px-3 py-1.5 font-mono text-sm tabular-nums text-[#e2e2e6] outline-none focus:border-[#ff8a50]"
            />
            <button
              onClick={saveFixed}
              disabled={savingPlan || amount < MIN_FIXED || amount > MAX_FIXED}
              className="rounded-lg bg-[#e8692a] px-4 py-2 text-xs font-bold text-white shadow-lg  hover:bg-[#e8692a]/90 disabled:opacity-50"
            >
          ...
          {currentCommission?.source === 'fixed' && (
            <button
              onClick={clearFixed}
              disabled={savingPlan}
              className="mt-3 flex w-full items-center justify-center gap-1.5 rounded-lg border border-[#49443f]/50 px-3 py-2 text-[11px] text-[#e2e2e6]/70 hover:border-[#ff5625]/40 hover:text-[#ff5625] disabled:opacity-50"
            >
      ...
      <div className="mt-8 overflow-hidden rounded-2xl border border-[#49443f]/40 bg-[#1a1c1f]">
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {orders.map((o) => (
              <tr key={o.id} className="border-t border-[#49443f]/40">
                ...
                <td className="px-4 py-3 text-xs uppercase tracking-wider text-[#ff8a50]">{o.status}</td>
              </tr>
            ))}
          </tbody>
        </table>
