      ...
      <div className="mb-8 rounded-2xl border border-[#3d3f41]/40 bg-[#1e2022] p-6">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            ...
            <input type="file" accept="application/pdf,image/jpeg,image/png,image/webp,video/mp4" onChange={(e) => setFile(e.target.files?.[0] ?? null)} className="block w-full text-sm text-[#e2e2e5]/70 file:mr-3 file:rounded-full file:border-0 file:bg-[#ff5625] file:px-4 file:py-2 file:text-sm file:font-medium file:text-white hover:file:bg-[#ff5625]" />
          </div>
        </div>
        <div className="mt-4">
          <button onClick={upload} disabled={busy} className="rounded-full bg-[#ff5625] px-6 py-2.5 text-sm font-medium text-white transition-colors glow-primary-hover hover:bg-[#ff5625]/90 disabled:opacity-50">
        ...
        <table className="w-full min-w-[560px] text-left text-sm">
          ...
          <tbody>
            {docs.length === 0 ? (
              ...
              <tr key={d.id} className="border-t border-[#3d3f41]/40 hover:bg-[#282a2c]/40">
                <td className="px-4 py-3 font-medium">{d.title}</td>
                <td className="px-4 py-3 text-[#e2e2e5]/70">{catLabel[d.category]}</td>
                <td className="px-4 py-3"><span className="rounded-full bg-[#282a2c] px-2 py-0.5 text-[10px] uppercase text-[#e2e2e5]/70">{visLabel[d.visible_to]}</span></td>
