...
export default function AdminUpgradePage() {
  ...
  return (
    ...
    >
      <div className="mb-6">
        <p className="text-[11px] uppercase tracking-[0.3em] text-[#8bd6b6]">{t('portal.admin.upgrade.eyebrow')}</p>
      ...
      <div className="mb-8 flex items-end gap-3">
        <div className="flex-1">
          ...
          <input
            ...
            className="w-full rounded-lg border border-[#3f4944]/40 bg-[#1a1c1f] px-3 py-2 text-sm outline-none focus:border-[#8bd6b6] font-mono tabular-nums"
          />
        </div>
        <button
          onClick={() => upgrade(userId)}
          disabled={busy}
          className="rounded-full bg-[#065f46] px-5 py-2.5 text-sm font-medium text-white transition-colors hover:bg-[#065f46]/90 disabled:opacity-50"
        >
      ...
      <div className="mb-8 rounded-2xl border border-[#3f4944]/40 bg-[#1a1c1f] p-4">
        ...
        <div className="flex flex-wrap items-end gap-3">
          <div className="min-w-[240px] flex-1">
            ...
            <input
              ...
              placeholder={t('portal.admin.upgrade.input.placeholder')}
              className="w-full rounded-lg border border-[#3f4944]/40 bg-[#1a1c1f] px-3 py-2 text-sm font-mono tabular-nums outline-none focus:border-[#8bd6b6]"
            />
          </div>
          <div>
            ...
            <select
              ...
              className="rounded-lg border border-[#3f4944]/40 bg-[#1a1c1f] px-3 py-2 text-sm outline-none focus:border-[#8bd6b6]"
            >
      ...
      <div className="overflow-x-auto portal-scroll rounded-2xl border border-[#3f4944]/40 bg-[#1a1c1f]">
        <table className="w-full min-w-[640px] text-left text-sm">
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className="border-t border-[#3f4944]/40 hover:bg-[#1e2023]/40">
                ...
                <td className="px-4 py-3">
                  <span className={`inline-flex rounded-md px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider ${
                    ...
                    : r.role === 'dealer' ? 'border border-[#3f4944]/60 bg-[#1e2023] text-[#a8b3ac]'
                    : 'text-[#a8b3ac]/60'
                  }`}>{r.role ?? '—'}</span>
                </td>
                <td className="px-4 py-3 text-right">
                  {r.role === 'dealer' && (
                    <button
                      ...
                      disabled={busy}
                      className="rounded-full border border-[#3f4944]/60 px-3 py-1.5 text-xs font-medium text-[#e2e2e6] hover:border-[#8bd6b6] hover:text-[#8bd6b6] disabled:opacity-50"
                    >
                      {t('portal.admin.upgrade.action.promote')}
                    </button>
                  )}
