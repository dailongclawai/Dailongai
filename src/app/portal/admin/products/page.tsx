...
export default function AdminProductsPage() {
  ...
  const input = 'w-full rounded-lg border border-[#3d3f41]/50 bg-[#1e2022] px-3 py-2 text-sm text-[#e2e2e5] placeholder:text-[#e2e2e5]/40 outline-none focus:border-[#ff5625]';
  ...
  return (
    <PortalShell variant="admin" nav={<AdminNav />}>
      <div className="mb-6">
        ...
        <p className="mt-2 text-sm text-[#e2e2e5]/60">Đại lý chỉ ghi nhận đơn cho các model đang bật.</p>
      </div>

      <div className="mb-8 rounded-2xl border border-[#3d3f41]/40 bg-[#1e2022] p-6">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="font-headline text-xl">{form.id ? 'Sửa sản phẩm' : 'Thêm sản phẩm mới'}</h2>
          {form.id && (
            <button onClick={() => setForm({ ...emptyForm })} className="text-xs text-[#e2e2e5]/60 hover:text-[#ff5625]">+ Thêm mới</button>
          )}
        </div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            <label className="mb-1 block text-xs uppercase tracking-wider text-[#e2e2e5]/70">Mã model</label>
            <input value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} placeholder="VD: ZHIDUN-CEO" className={`${input} font-mono`} />
          </div>
          <div>
            <label className="mb-1 block text-xs uppercase tracking-wider text-[#e2e2e5]/70">Tên sản phẩm</label>
            <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="VD: ZhiDun CEO" className={input} />
          </div>
          <div>
            <label className="mb-1 block text-xs uppercase tracking-wider text-[#e2e2e5]/70">Giá niêm yết (đ)</label>
          ...
          <div className="flex items-end gap-3">
            <label className="flex items-center gap-2 text-sm text-[#e2e2e5]/80">
          ...
          <div className="sm:col-span-2">
            <label className="mb-1 block text-xs uppercase tracking-wider text-[#e2e2e5]/70">Mô tả</label>
        ...
        <div className="mt-4">
          <button onClick={save} disabled={busy} className="rounded-full bg-[#ff5625] px-6 py-2.5 text-sm font-medium text-white transition-colors glow-primary-hover hover:bg-[#ff5625]/90 disabled:opacity-50">
      ...
      <div className="overflow-x-auto overflow-hidden rounded-2xl border border-[#3d3f41]/40 bg-[#1e2022]">
        <table className="w-full min-w-[560px] text-left text-sm">
          ...
          <tbody>
            {models.length === 0 ? (
              <tr><td colSpan={5} className="px-4 py-10 text-center text-[#e2e2e5]/50">Chưa có sản phẩm. Thêm model đầu tiên ở trên.</td></tr>
            ) : models.map((m) => (
              <tr key={m.id} className="border-t border-[#3d3f41]/40 hover:bg-[#282a2c]/40">
                <td className="px-4 py-3 font-mono text-[#e2e2e5]/80">{m.code}</td>
                <td className="px-4 py-3 font-medium">{m.name}</td>
                <td className="px-4 py-3 text-right font-mono tabular-nums">{fmtVnd(Number(m.base_price))}</td>
                <td className="px-4 py-3 text-center">
                  <span className={`rounded-full px-2 py-0.5 text-[10px] font-medium uppercase ${m.active ? 'bg-[#34d399]/15 text-[#34d399]' : 'bg-[#282a2c] text-[#e2e2e5]/50'}`}>
                    {m.active ? 'Đang bán' : 'Ẩn'}
                  </span>
                </td>
                <td className="px-4 py-3 text-right">
                  <button onClick={() => edit(m)} className="mr-3 text-xs text-[#ff5625] hover:underline">Sửa</button>
                  <button onClick={() => toggle(m)} disabled={busy} className="text-xs text-[#e2e2e5]/60 hover:text-[#e2e2e5] disabled:opacity-50">
                    {m.active ? 'Ẩn' : 'Hiện'}
                  </button>
                </td>
              </tr>
