...
export default function ProfilePage() {
  ...
  return (
    <PortalShell variant={profile.role ?? 'dealer'} nav={nav}>
      <div className="mb-6">
        <p className="text-[11px] uppercase tracking-[0.3em] text-[#ffb5a1]">Hồ sơ</p>
        <h1 className="mt-2 font-headline text-3xl">Thông tin tài khoản</h1>
      </div>
      <div className="grid gap-6 md:grid-cols-2">
        <form onSubmit={saveProfile} className="space-y-4 rounded-2xl border border-[#5b4039]/40 bg-[#2c1c17] p-6 backdrop-blur">
          <h2 className="text-base font-semibold">Thông tin</h2>
          <div>
            <label className="mb-1 block text-xs uppercase tracking-wider text-[#fadcd5]/60">Email</label>
            <input
              value={profile.email ?? ''}
              disabled
              className="w-full rounded-lg border border-[#5b4039]/40 bg-[#2c1c17] px-3 py-2 text-sm text-[#fadcd5]/60"
            />
          </div>
          <div>
            <label className="mb-1 block text-xs uppercase tracking-wider text-[#fadcd5]/60">Họ tên</label>
            <input
              value={fullName}
              onChange={(e) => setFullName(e.target.value)}
              className="w-full rounded-lg border border-[#5b4039]/50 bg-[#2c1c17] px-3 py-2 text-sm text-[#fadcd5] placeholder:text-[#fadcd5]/40 focus:border-[#ffb5a1] outline-none"
            />
          </div>
          <div>
            <label className="mb-1 block text-xs uppercase tracking-wider text-[#fadcd5]/60">Số điện thoại</label>
            <input
              value={phone}
          ...
          <button
            type="submit"
            disabled={busy}
            className="rounded-full bg-[#ff5626] px-5 py-2 text-sm font-medium text-white glow-primary-hover hover:bg-[#ff5626]/90 disabled:opacity-50"
          >
            Lưu
          </button>
        </form>
        <form onSubmit={changePassword} className="space-y-4 rounded-2xl border border-[#5b4039]/40 bg-[#2c1c17] p-6 backdrop-blur">
          <h2 className="text-base font-semibold">Đổi mật khẩu</h2>
          <div>
            <label className="mb-1 block text-xs uppercase tracking-wider text-[#fadcd5]/60">Mật khẩu mới</label>
            <input
              type="password"
              value={newPassword}
        ...
        <form onSubmit={saveCompliance} className="space-y-4 rounded-2xl border border-[#5b4039]/40 bg-[#2c1c17] p-6 backdrop-blur md:col-span-2">
          <div className="flex items-baseline justify-between">
            ...
            <span className="text-[11px] text-[#fadcd5]/40">Phục vụ hợp đồng &amp; pháp lý</span>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label className="mb-1 block text-xs uppercase tracking-wider text-[#fadcd5]/60">Tên doanh nghiệp / hộ kinh doanh</label>
              <input value={bizName} onChange={(e) => setBizName(e.target.value)} placeholder="VD: Hộ KD Trần Thị A" className="w-full rounded-lg border border-[#5b4039]/50 bg-[#2c1c17] px-3 py-2 text-sm text-[#fadcd5] placeholder:text-[#fadcd5]/40 focus:border-[#ffb5a1] outline-none" />
            </div>
            <div>
              <label className="mb-1 block text-xs uppercase tracking-wider text-[#fadcd5]/60">Mã số thuế / CCCD</label>
              <input value={idNumber} onChange={(e) => setIdNumber(e.target.value)} placeholder="Mã số thuế hoặc CCCD" className="w-full rounded-lg border border-[#5b4039]/50 bg-[#2c1c17] px-3 py-2 text-sm text-[#fadcd5] placeholder:text-[#fadcd5]/40 focus:border-[#ffb5a1] outline-none font-mono tabular-nums" />
            </div>
            <div className="sm:col-span-2">
              <label className="mb-1 block text-xs uppercase tracking-wider text-[#fadcd5]/60">Địa chỉ kinh doanh</label>
              <input value={bizAddress} onChange={(e) => setBizAddress(e.target.value)} placeholder="Địa chỉ" className="w-full rounded-lg border border-[#5b4039]/50 bg-[#2c1c17] px-3 py-2 text-sm text-[#fadcd5] placeholder:text-[#fadcd5]/40 focus:border-[#ffb5a1] outline-none" />
            </div>
          </div>
          <button type="submit" disabled={busy} className="rounded-full bg-[#ff5626] px-5 py-2 text-sm font-medium text-white glow-primary-hover hover:bg-[#ff5626]/90 disabled:opacity-50">
