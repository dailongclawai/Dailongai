...
export default function OnboardingPage() {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center px-4 py-10">
      <div className="portal-glass w-full max-w-md space-y-6 rounded-3xl border border-[#5b4039]/40 p-10">
        <div className="text-center">
          <p className="text-[11px] uppercase tracking-[0.3em] text-[#ffb5a1]">Bước cuối</p>
          <h1 className="mt-3 font-headline text-3xl">Hoàn tất hồ sơ</h1>
          <p className="mt-2 text-sm text-[#fadcd5]/60">Chỉ cần số điện thoại để admin liên hệ xác minh</p>
        </div>
        <form onSubmit={submit} className="space-y-5">
          <div>
            <label htmlFor="phone" className="mb-1 block text-xs uppercase tracking-wider text-[#fadcd5]/60">
              Số điện thoại
            </label>
            <input
              id="phone"
          ...
          <p className="rounded-lg bg-[#2c1c17] px-4 py-3 text-xs text-[#fadcd5]/60">
            Loại tài khoản (đại lý / supervisor) sẽ do admin Đại Long gán khi duyệt hồ sơ.
          </p>
          <button
            type="submit"
            disabled={submitting}
            className="w-full rounded-full bg-[#ff5626] py-3 text-sm font-medium text-white glow-primary-hover hover:bg-[#ff5626]/90 disabled:opacity-50"
          >
