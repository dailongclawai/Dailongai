...
export default function RegisterPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (data.session) {
      ...
      toast.success('Đã gửi email xác nhận. Mở email để hoàn tất đăng ký.');
    }
  };
      ...
      <div className="w-full max-w-md space-y-6 rounded-2xl border border-[#ff5625]/20 bg-gradient-to-br from-[#ff5625]/[0.04] via-[#11151a] to-[#11151a] p-7 sm:p-9">
        ...
        {ref && (
          <p className="rounded-lg border border-[#10b981]/30 bg-[#10b981]/[0.08] px-3 py-2 text-xs text-[#10b981]">
            Đang đăng ký qua lời mời của quản lý — tài khoản tự thuộc nhánh của họ
          </p>
        )}
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <div>
            <label htmlFor="password" className="mb-1.5 block text-xs text-[#9ca3af]">Mật khẩu</label>
            <PasswordInput
              id="password"
              value={password}
          ...
          >
            ...
            {busy ? 'Đang tạo tài khoản…' : 'Tiếp tục'}
          </button>
        </form>
