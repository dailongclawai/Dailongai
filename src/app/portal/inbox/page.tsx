...
const SEVERITY_META: Record<NotificationSeverity, { stripe: string; icon: string; iconBg: string; iconBorder: string }> = {
  ...
  success:  { stripe: '#ff5625', icon: 'check_circle', iconBg: 'bg-orange-500/10', iconBorder: 'border-orange-500/30' },
  warning:  { stripe: '#ff8a50', icon: 'warning',      iconBg: 'bg-[#ff8a50]/10',  iconBorder: 'border-[#ff8a50]/30' },
  ...
  return (
    <PortalShell variant={profile.role ?? 'dealer'}>
      <div className="mb-8 flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-[11px] uppercase tracking-[0.3em] text-[#b3aca8]">{t('portal.inbox.eyebrow')}</p>
          <h1 className="mt-2 font-headline text-3xl">{t('portal.inbox.title')}</h1>
        </div>
        <button
          onClick={onMarkAllRead}
          disabled={busy || counts.unread === 0}
          className="flex items-center gap-2 rounded-lg border border-[#49443f]/40 bg-[#1a1c1f] px-5 py-2.5 text-sm font-medium text-[#b3aca8] transition-colors hover:bg-[#1e2023] hover:text-[#e2e2e6] disabled:opacity-50"
        >
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
        <aside className="lg:col-span-3">
          <div className="lg:sticky lg:top-24 overflow-hidden rounded-xl border border-[#49443f]/40 bg-[#1a1c1e]">
            <div className="border-b border-[#49443f]/20 bg-[#1e2023]/50 px-4 py-3">
              <span className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#ff8a50]">{t('portal.inbox.filter.header')}</span>
            </div>
                ...
                return (
                  <button
                    ...
                    className={`flex w-full items-center justify-between rounded-lg px-3 py-2.5 text-sm transition-colors ${
                      active
                        ? 'border-l-2 border-[#ff8a50] bg-[#ff8a50]/10 text-[#ff8a50]'
                        : 'text-[#b3aca8] hover:bg-[#1e2023] hover:text-[#e2e2e6]'
                    }`}
                  >
                    ...
                    {n > 0 && (
                      <span className={`rounded px-1.5 py-0.5 text-[10px] font-bold ${
                        active ? 'bg-[#ff8a50]/20 text-[#ff8a50]' : 'bg-[#3d3f41]/40 text-[#b3aca8]'
                      }`}>
                        {n}
                      </span>
                    )}
        ...
        <div className="space-y-3 lg:col-span-9">
          {filtered.length === 0 ? (
            <div className="rounded-xl border border-[#49443f]/30 bg-[#1a1c1e] py-16 text-center opacity-60">
              <span className="material-symbols-outlined text-[48px] text-[#b3aca8]">inbox</span>
              <p className="mt-3 text-sm text-[#b3aca8]">{t('portal.inbox.empty')}</p>
            </div>
          ) : filtered.map((m) => {
            ...
            return (
              <div
                ...
                className={`group relative flex cursor-pointer items-start gap-4 overflow-hidden rounded-xl border p-5 transition-all hover:bg-[#1a1c1f] ${
                  unread ? 'border-[#49443f]/40 bg-[#1a1c1e]' : 'border-[#49443f]/20 bg-[#1a1c1e]/50 opacity-80'
                }`}
              >
                {unread && (
                  <span
                ...
                <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-lg border ${
                  unread ? `${sev.iconBg} ${sev.iconBorder}` : 'border-[#49443f]/30 bg-[#1e2023]'
                }`}>
                ...
                <div className="min-w-0 flex-1">
                  <div className="mb-1 flex items-center gap-3">
                    <h3 className={`truncate font-headline text-base ${unread ? 'text-[#e2e2e6]' : 'text-[#b3aca8]'}`}>{m.subject}</h3>
                    {unread && <span className="h-2 w-2 shrink-0 animate-pulse rounded-full bg-[#ff8a50]" />}
                  </div>
                  <p className={`mb-3 line-clamp-3 text-sm leading-relaxed ${unread ? 'text-[#b3aca8]' : 'text-[#b3aca8]/70'}`}>{m.body}</p>
                  <div className="flex items-center gap-5 text-[11px] text-[#b3aca8]/80">
                  ...
                  <div className="shrink-0 pt-1" onClick={(e) => e.stopPropagation()}>
                    <Link
                      ...
                      className={`inline-block rounded-lg px-4 py-2 text-sm font-bold transition-all active:scale-95 ${
                        unread
                          ? 'bg-[#e8692a] text-white shadow-lg  hover:bg-[#e8692a]/90'
                          : 'border border-[#49443f]/40 bg-[#1e2023] text-[#b3aca8] hover:bg-[#3d3f41]'
                      }`}
                    >
                      {m.action_label}
                    </Link>
