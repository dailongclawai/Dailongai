  ...
  return (
    <PortalShell variant={variantFor()}>
      <div className="mb-8">
        ...
        <h1 style={display} className="mt-2 text-3xl font-light italic">Hộp thư</h1>
      </div>

      {/* Message list */}
      <div className="mb-10 overflow-hidden rounded-2xl border border-[#0e1525]/15 bg-white/80">
        {messages.length === 0 ? (
          <p className="px-6 py-10 text-center text-sm text-[#0e1525]/40">Hộp thư trống</p>
        ) : (
                    ...
                    {openId === msg.id && (
                      <div onClick={(e) => e.stopPropagation()}>
                        ...
                        {isAdmin && msg.sender_id && (
                          <div className="mt-4 space-y-2 border-t border-[#0e1525]/10 pt-4">
                            <p className="text-[10px] uppercase tracking-[0.2em] text-[#5d8d6a]">Trả lời</p>
                            <textarea
                            ...
                            <button
                              ...
                              className="rounded-full bg-[#5d8d6a] px-5 py-2 text-xs font-medium text-white transition-colors hover:bg-[#4a7357] disabled:opacity-50"
                            >
      ...
      {!isAdmin && (
        <section>
          ...
          >
            <div>
              <label className="mb-1 block text-xs uppercase tracking-wider text-[#0e1525]/50">Tiêu đề</label>
              <input
                type="text"
                value={subject}
            ...
            <div>
              <label className="mb-1 block text-xs uppercase tracking-wider text-[#0e1525]/50">Nội dung</label>
              <textarea
                value={body}
