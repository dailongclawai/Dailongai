...
function statusOf(r: LedgerRow, t: (k: string) => string): { label: string; cls: string; dot: string; bucket: Exclude<Bucket, 'all'> } {
  ...
  if (r.status === 'voided' || r.commission?.voided_at) return { label: t('portal.dealer.commission.status.voided'), cls: 'text-[#b3aca8] bg-[#1e2023] border-[#49443f]/40', dot: 'bg-[#a0a0a8]', bucket: 'rejected' };
  if (r.commission?.paid_at) return { label: t('portal.dealer.commission.status.paid'), cls: 'text-orange-400 bg-orange-500/10 border-orange-500/20', dot: 'bg-orange-400', bucket: 'paid' };
  ...
  if (r.status === 'paid') return { label: t('portal.dealer.commission.status.paid'), cls: 'text-orange-400 bg-orange-500/10 border-orange-500/20', dot: 'bg-orange-400', bucket: 'paid' };
  return { label: t('portal.dealer.commission.status.processing'), cls: 'text-[#b3aca8] bg-[#1e2023] border-[#49443f]/40', dot: 'bg-[#a0a0a8]', bucket: 'pending' };
}
...
function planLabel(pct: number | null, t: (k: string) => string): { name: string; cls: string } {
  if (pct === null) return { name: t('portal.dealer.commission.plan.estimated'), cls: 'bg-[#1e2023] text-[#b3aca8] border-[#49443f]/40' };
  ...
  return { name: `${t('portal.dealer.commission.plan.standard')} ${pct}%`, cls: 'bg-[#ff8a50]/10 text-[#ff8a50] border-[#ff8a50]/20' };
}
  ...
  return (
    <PortalShell variant="dealer">
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-[11px] uppercase tracking-[0.3em] text-[#ff8a50]">{t('portal.dealer.commission.eyebrow')}</p>
          <h1 className="mt-1 font-headline text-3xl">{t('portal.dealer.commission.title')}</h1>
          <p className="mt-1 text-sm text-[#b3aca8]">{t('portal.dealer.commission.subtitle')}</p>
        </div>
      ...
      <div className="portal-glass mb-8 grid grid-cols-1 gap-4 rounded-2xl p-4 sm:grid-cols-12 sm:items-end">
        <div className="sm:col-span-3">
          <label className="mb-1.5 ml-1 block text-[11px] uppercase tracking-wider text-[#b3aca8]">{t('portal.dealer.commission.filter.status_label')}</label>
        ...
        <div className="sm:col-span-4">
          <label className="mb-1.5 ml-1 block text-[11px] uppercase tracking-wider text-[#b3aca8]">{t('portal.dealer.commission.filter.date_range')}</label>
        ...
        <div className="sm:col-span-5">
          <label className="mb-1.5 ml-1 block text-[11px] uppercase tracking-wider text-[#b3aca8]">{t('portal.dealer.commission.filter.search_label')}</label>
          <div className="relative">
            <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-[20px] text-[#b3aca8]">search</span>
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder={t('portal.dealer.commission.filter.search_placeholder')} className="w-full rounded-xl border border-[#49443f]/40 bg-[#1a1c1e] py-2.5 pl-11 pr-4 text-sm text-[#e2e2e6] placeholder:text-[#b3aca8]/40 focus:ring-1 focus:ring-[#ff8a50] outline-none" />
          </div>
        </div>
      </div>
      ...
      {fetching ? (
        ...
        <div className="space-y-8">
          {groupedByMonth.map((group) => (
            <section key={group.key}>
              <header className="sticky top-[64px] z-10 -mx-2 mb-3 flex items-baseline justify-between gap-3 rounded-md bg-[#0c0e11]/95 px-2 py-2 backdrop-blur">
                ...
                <p className="text-xs text-[#b3aca8]">
                  <span className="font-mono font-semibold tabular-nums text-[#ff8a50]">{fmtVnd(group.sold)} ₫</span>
                  {' · '}
                  ...
                  return (
                    <li key={r.id}>
                      <button
                        ...
                        className="w-full rounded-2xl border border-[#49443f] bg-[#1a1c1f] p-4 text-left transition-colors hover:border-[#ff8a50]/40 hover:bg-[#1e2023] active:scale-[0.998]"
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0 flex-1">
                            <p className="text-sm font-semibold text-[#e2e2e6] truncate">{r.customer_name}</p>
                            <p className="mt-0.5 text-xs text-[#b3aca8]">
                              <span className="font-mono tabular-nums">{new Date(r.sale_date).toLocaleDateString('vi-VN')}</span>
                              {' · '}
                              <span className="font-mono tabular-nums text-[#ff8a50]">{r.serial_number}</span>
                            </p>
                          </div>
                        ...
                        <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3">
                          <div>
                            <p className="text-[10px] uppercase tracking-wider text-[#b3aca8]">{t('portal.dealer.commission.card.sale_price')}</p>
                            <p className="font-headline text-lg tabular-nums">{fmtVnd(Number(r.sale_price))} ₫</p>
                          </div>
                          <div>
                            <p className="text-[10px] uppercase tracking-wider text-[#b3aca8]">{t('portal.dealer.commission.card.commission')}</p>
                            <p className={`font-headline text-lg tabular-nums ${commissionAmount && r.commission?.paid_at ? 'text-[#ff5625]' : commissionAmount ? 'text-[#3b82f6]' : 'text-[#b3aca8]'}`}>
                              {commissionAmount !== null ? `${fmtVnd(commissionAmount)} ₫` : '—'}
                            </p>
                          </div>
                          <div className="col-span-2 sm:col-span-1">
                            <p className="text-[10px] uppercase tracking-wider text-[#b3aca8]">{t('portal.dealer.commission.card.plan')}</p>
                            <span className={`mt-1 inline-flex rounded-md border px-2 py-0.5 text-[11px] font-bold ${plan.cls}`}>{plan.name}</span>
                          </div>
                        </div>
                        {open && (
                          <div className="mt-4 grid grid-cols-2 gap-4 border-t border-[#49443f] pt-4 sm:grid-cols-4">
                            <div>
                              <p className="text-[10px] uppercase text-[#b3aca8]">{t('portal.dealer.commission.card.order_date')}</p>
                              <p className="text-sm">{new Date(r.sale_date).toLocaleDateString('vi-VN')}</p>
                            </div>
                            <div>
                              <p className="text-[10px] uppercase text-[#b3aca8]">{t('portal.dealer.commission.card.plan')}</p>
                              <p className="text-sm">{plan.name}</p>
                            </div>
                            <div>
                              <p className="text-[10px] uppercase text-[#b3aca8]">{t('portal.dealer.commission.card.payout_date')}</p>
                            ...
                            <div className="col-span-2 sm:col-span-1">
                              <p className="text-[10px] uppercase text-[#b3aca8]">{t('portal.dealer.commission.card.tx_id')}</p>
                              <p className="font-mono text-xs text-[#ff8a50] break-all">{r.commission?.payment_proof_url || '—'}</p>
                            </div>
                          </div>
                        )}
                      </button>
          ...
          <p className="px-2 text-xs text-[#b3aca8]">
