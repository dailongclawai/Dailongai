...
function statusOf(r: LedgerRow): { label: string; cls: string } {
  ...
  if (r.status === 'voided' || r.commission?.voided_at) return { label: 'Đã hủy', cls: 'bg-white/10 text-[#e2e2e5]/50' };
  if (r.commission?.paid_at) return { label: 'Đã thanh toán', cls: 'bg-[#34d399]/15 text-[#34d399]' };
  if (r.commission) return { label: 'Đã duyệt · chờ chi', cls: 'bg-[#ff5625]/15 text-[#ff8a5c]' };
  if (r.status === 'pending') return { label: 'Chờ duyệt', cls: 'bg-[#ffc033]/15 text-[#ffc033]' };
  return { label: r.status, cls: 'bg-white/10 text-[#e2e2e5]/60' };
}
  ...
  return (
    <PortalShell variant="dealer" nav={<DealerNav />}>
      <div className="mb-6">
        ...
        <p className="mt-2 text-sm text-[#e2e2e5]/60">Chi tiết từng đơn: hoa hồng phát sinh, trạng thái duyệt và lịch thanh toán.</p>
      </div>

      <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
        {[
          { label: 'Tổng hoa hồng phát sinh', value: totalAccrued, tone: 'text-[#e2e2e5]' },
          { label: 'Đã duyệt · chờ chi', value: pendingPay, tone: 'text-[#ff8a5c]' },
          { label: 'Đã thanh toán', value: paid, tone: 'text-[#34d399]' },
        ].map((k) => (
          <div key={k.label} className="rounded-xl border border-white/10 bg-[#1e2022] p-5">
      ...
      <div className="overflow-x-auto overflow-hidden rounded-2xl border border-white/12 bg-[#1e2022]">
        <table className="w-full min-w-[720px] text-left text-sm">
          ...
          <tbody>
            {rows.length === 0 ? (
              <tr><td colSpan={7} className="px-4 py-10 text-center text-[#e2e2e5]/50">Chưa có đơn nào. Ghi nhận đơn để bắt đầu tích hoa hồng.</td></tr>
            ) : rows.map((r) => {
              const st = statusOf(r);
              return (
                <tr key={r.id} className="border-t border-white/12 hover:bg-white/5">
                  <td className="px-4 py-3 font-mono tabular-nums text-xs text-[#e2e2e5]/60">{new Date(r.sale_date).toLocaleDateString('vi-VN')}</td>
                  <td className="px-4 py-3 font-mono text-xs text-[#e2e2e5]/80">{r.serial_number}</td>
                  ...
                  <td className="px-4 py-3 text-xs text-[#e2e2e5]/60">
